...
if cfg.enableTiltAlign && cfg.tiltResidualRefineEnable && abs(rotDeg) > 0
    ...
    if isfinite(residualTiltDeg) && abs(residualTiltDeg) >= cfg.tiltResidualRefineThresholdDeg
        ...
        upAligned = localRotateComplex(up, rotDeg);
        [residualTiltDeg, residualTiltInfo] = localEstimateTilt(upAligned, cfg, residualSearch);
        residualTiltInfo.used = 'residual-check+post';
    end
end
...
info = struct();
info.method = 'separability-search(coarse-mid-fine)';
info.mainPoints = 0;
info.sidePoints = 0;
info.pcaRawDeg = NaN;
...
evalImg = localBuildTiltEvalImage(evalPatch, cfg.tiltEvalDbFloor);
...
limits = [-45, 45];
searchCfg = localResolveTiltSearchCfg(cfg, searchOverride, limits);

[coarseAngle, ~] = localSearchBestAngle(evalImg, ...
    localAngleGrid(searchCfg.centerDeg, searchCfg.coarseHalfRangeDeg, ...
    searchCfg.coarseStepDeg, limits));
[midAngle, ~] = localSearchBestAngle(evalImg, ...
    localAngleGrid(coarseAngle, searchCfg.midHalfRangeDeg, ...
    searchCfg.midStepDeg, limits));
[fineAngle, fineScore, fineAngles, fineScores, fineIndex] = localSearchBestAngle(evalImg, ...
    localAngleGrid(midAngle, searchCfg.fineHalfRangeDeg, ...
    searchCfg.fineStepDeg, limits));
[refinedAngle, ~] = localParabolicAngleRefine(fineAngles, fineScores, fineIndex);

if ~isfinite(refinedAngle)
    refinedAngle = fineAngle;
    refinedScore = fineScore;
else
    refinedScore = localTiltScore(evalImg, refinedAngle);
end
...
function searchCfg = localResolveTiltSearchCfg(cfg, override, limits)
searchCfg = struct();
searchCfg.centerDeg = 0;
searchCfg.coarseHalfRangeDeg = min(abs(diff(limits)) / 2, 45);
searchCfg.coarseStepDeg = cfg.tiltSearchCoarseStepDeg;
...
function evalImg = localBuildTiltEvalImage(ampPatch, dbFloor)
...
rotImg(rotImg < 0) = 0;
innerSize = localInnerCropSize(size(rotImg, 1));
